...
def pull_choice_sets():
    ...
    for cs in raw:
        ...
        for ch in cs.get("extra_choices", []):
            # NetBox API may return choices as:
            #   list:   ["CIS-WS2022", "CIS Windows Server 2022"]
            #   dict:   {"value": "CIS-WS2022", "label": "CIS Windows Server 2022"}
            #   string: "['CIS-WS2022', 'CIS Windows Server 2022']"  ← API quirk
            if isinstance(ch, list) and len(ch) == 2:
                ...
                # Handle string-encoded list: "['value', 'label']"
                import ast
                try:
                    parsed = ast.literal_eval(ch)
                    if isinstance(parsed, (list, tuple)) and len(parsed) >= 1:
                        choices.append({"value": parsed[0], "label": parsed[1] if len(parsed) > 1 else parsed[0]})
                    elif isinstance(parsed, dict):
                        choices.append({"value": parsed.get("value", ""), "label": parsed.get("label", "")})
                except (ValueError, SyntaxError):
                    choices.append({"value": ch, "label": ch})
...
<div class="container">
  ...
  <div class="section">
    ...
    <div class="section-body">
      ...
      <div class="field-row">
        <label>MAC Address
          <span class="hint">Management interface<br>AA:BB:CC:DD:EE:FF</span>
        </label>
        <input type="text" id="mac_address" placeholder="e.g. AA:BB:CC:DD:EE:FF">
      </div>
      <div class="field-row">
...
function clearForm() {{
  ['hostname','ip_address','mac_address','department','appliance_vendor','specific_product_text','notes']
...
function exportExcel() {{
  ...
  const COLS = [
    'hostname','ip_address','mac_address','asset_type','os','environment','sensitive_data','site','department',
