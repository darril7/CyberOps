...
from runner.orphan_collector import OrphanCollector
from runner.orphan_report import save_reports
...
async def soc_inventory(
    ...
    dry_run: bool = False,
    report_orphans: bool = True,        # generate orphan report after run
    reports_dir: str = "reports",       # where to save reports
):
    ...
    # ── Orphan summary ────────────────────────────────────────────────────
    orphan_summary = orphans.summary()
    total_orphans = orphan_summary["total"]
    ...
    if total_orphans > 0:
        logger.warning(
            f"Orphan devices by client: {orphan_summary['by_client']}"
        )
        logger.warning(
            f"Orphan devices by tool:   {orphan_summary['by_tool']}"
        )

    # ── Generate orphan report ─────────────────────────────────────────────
    html_path = csv_path = None
    if report_orphans and not dry_run:
        try:
            import prefect
            run_id = str(prefect.runtime.flow_run.id)
        except Exception:
            run_id = "unknown"

        if total_orphans > 0:
            html_path, csv_path = save_reports(
                orphans,
                output_dir=reports_dir,
                run_id=run_id,
            )
            logger.info(f"Orphan report saved: {html_path}")
            logger.info(f"Orphan CSV saved:    {csv_path}")
        else:
            logger.info("No orphan devices — report skipped")
    ...
    return {
        ...
        "orphan_by_client": orphan_summary["by_client"],
        "orphan_by_tool":   orphan_summary["by_tool"],
        "orphan_report_html": str(html_path) if html_path else None,
        "orphan_report_csv":  str(csv_path)  if csv_path  else None,
    }
