...
def _parse_system(sys: dict) -> tuple[str | None, dict[str, Any]]:
    ...
    record: dict[str, Any] = {
        # Fields the runner will write to Netbox custom fields
        "trellix_last_seen":  last_seen,
        "trellix_av_status":  _parse_managed_state(managed_state),
        "trellix_dat_date":   dat_date,
        "trellix_agent_ver":  (
            props.get("AgentVersion") or props.get("agentVersion")
        ),
        # Match key helpers — used by netbox_client matcher
        "ip_address":  ip,
