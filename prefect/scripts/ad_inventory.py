...
def _fetch_sync(
    ...
    for entry in conn.entries:
        ...
        results[hostname.lower()] = {
            "ad_last_seen":      _parse_timestamp(entry, "lastLogonTimestamp"),
            "ad_os":             _str(entry, "operatingSystem"),
            "ad_os_version":     _str(entry, "operatingSystemVersion"),
            "ad_ou":             _parse_ou(entry),
            "ad_enabled":        _parse_enabled(entry),
            "ad_description":    _str(entry, "description"),
            "ad_created":        _str(entry, "whenCreated"),
            "ad_dns_name":       _str(entry, "dNSHostName"),
            "ad_distinguished_name": _str(entry, "distinguishedName"),
        }
